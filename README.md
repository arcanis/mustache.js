...
If you'd like to change HTML-escaping behavior globally (for example, to template non-HTML formats), you can override Mustache's escape function. For example, to disable all escaping: `Mustache.defaultConfig.escape = function(text) {return text;};`.
...
The `Mustache.defaultConfig.tags` property holds an array consisting of the opening and closing tag values. Set custom values by passing a new array of tags to `render()`, which gets honored over the default values, or by overriding the `Mustache.defaultConfig.tags` property itself:
...
```js
Mustache.defaultConfig.tags = customTags;
